export class Validators {
    ...
    static isValidInteger = (expression : number) => {
    ...
    static StringValidator = (
        fieldName: string, expression: string, required: boolean, type: string, minLength?: number, maxLength?: number
    ) : string[] => {
        const errorMessage = [];
        ...
        if (!type || type === 'text') {
            ...
            if (!Validators.EmailValidation(expression))
